...
export default function BookingForm() {
  const [form, setForm] = useState({
    ...
    phone: "",
    count: 1,
    userType: "normal", // "student" | "normal"
  });
  ...
  const ticketPrice = PRICES[form.userType];
  ...
  function handleChange(e) {
    ...
    if (
      ...
      toast.success(
        `🎉 You unlocked ${Math.floor(Number(value) / GROUP_THRESHOLD)} FREE ticket${
          Math.floor(Number(value) / GROUP_THRESHOLD) > 1 ? "s" : ""
        }!`,
        { duration: 5000 }
      );
    }
  ...
  async function onPaymentSuccess(response) {
    ...
    try {
      await emailjs.send(
        ...
        EMAILJS_TEMPLATE_ID,
        {
          ...
          user_type: form.userType === "student" ? "College Student" : "General Public",
    ...
    setSuccess({ ticketId, count, freeTickets, total, ticketPrice, userType: form.userType, paymentId: response.razorpay_payment_id || "N/A" });
    setLoading(false);
  }
  ...
  if (success) {
    return (
      ...
      >
        ...
        <h3 className="font-orbitron text-2xl font-black text-white mb-2">YOU'RE IN THE RACE!</h3>
        <p className="text-gray-400 font-rajdhani mb-6">Booking confirmed. Check your email for your ticket.</p>
          ...
          <div className="bg-black/70 rounded-lg p-4 space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-white font-bold font-rajdhani">
                {success.userType === "student" ? "🎓 College Student" : "👤 General Public"}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-400 font-rajdhani">Price / Ticket</span>
              <span className="text-green-400 font-bold">₹{success.ticketPrice}</span>
            </div>
            <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => { setSuccess(null); setForm({ name: "", email: "", phone: "", count: 1, userType: "normal" }); }}
  ...
  return (
    ...
    >
      ...
      {/* ── User type toggle ── */}
      <div className="mb-5">
        <label className="font-rajdhani text-sm text-gray-300 mb-2 block">I am a…</label>
          ...
          <button
            ...
            className={`relative rounded-xl p-4 border-2 text-left transition-all duration-200 ${
              form.userType === "student"
                ? "border-red-500 bg-red-600/15 glow-red"
                : "border-gray-700 bg-white/5 hover:border-gray-500"
            }`}
          >
            <div className="text-2xl mb-1">🎓</div>
            <p className="font-orbitron text-white text-xs font-bold">College Student</p>
            <p className="font-rajdhani text-green-400 font-black text-xl mt-0.5">₹99</p>
            <p className="font-rajdhani text-gray-500 text-xs line-through">₹999</p>
            {form.userType === "student" && (
              <span className="absolute top-2 right-2 w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold">✓</span>
            )}
          </button>
      ...
      <div className="mt-6 glass-card rounded-xl p-4 space-y-2 border border-yellow-500/20">
        <div className="flex justify-between font-rajdhani text-gray-400 text-sm">
          ...
          <span className="text-white">
            {form.userType === "student" ? "🎓 College Student" : "👤 General Public"}
          </span>
        </div>
        <div className="flex justify-between font-rajdhani text-gray-400 text-sm">
        ...
        {freeTickets > 0 && (
          <div className="flex justify-between font-rajdhani text-yellow-400 text-sm">
            <span>Total tickets (incl. free)</span>
